...
is the keyboard device on the host machine whereas `/dev/input/event1` is the keyboard device on the client
...
(https://github.com/ritiek/netevent/tree/termux). You must need root to access `/dev/input/event*` files on
...
created on my laptop and the touch on the laptop was being registered when I `cat` the appropriate device
event file. Seems like my laptop has no idea what the event data it received is supposed to do. There might
