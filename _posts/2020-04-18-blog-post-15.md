...
<p align="center">*Here's how the machine looks like*</p>
...
<img src="https://i.imgur.com/rxACrpQ.jpg" width="290">
<p align="center">*The GRUB command-line*</p>
...
formatted partition. I've noticed that Pop!\_OS contains core image files contain symlinks to
other directories in the image and `FAT` systems doesn't support symlinks. So, in such a case
I'd create a complete copy of the file/directory and replace that its symlink. Done that. Now
it doesn't show up in the boot menu? Ok, put in a `bootia32.efi`. However, simply doing this
didn't succeed this time, it kept getting stuck somewhere doing `casper-prefetch` while loading
the kernel and would then drop me into a busybox shell, as I could see on my screen. This USB
did boot up on other my other machine with a 64-bit UEFI. So on such a machine, I checked the
default kernel version that Pop!\_OS comes with using:
...
Although I don't remember what error it came up with when the kernel was being initialized,
...
hardware. My friend had an Mint 19 ISO image laying around. so he sent `vmlinuz` and `initrd` files
